...
TestThread()
{
    ...
    while (t->getBurstT() > 0) {
        t->setBurstT(t->getBurstT() - 1); // CPU burst time decrease
        cout << "Thread " << t->getName() << " , remaining " << t->getBurstT() << endl;
...
Thread::SelfTest()
{
    ...
    int priority[thread_num] = {6, 3, 9 , 4, 1, 10};
    int burst[thread_num] = {6, 5, 4 ,2, 3, 1};
    int start[thread_num] = {1, 3 ,6 ,5, 10, 2};
    ...
    for (i = 0; i < thread_num; i ++) {
        ...
        t->setBurstT(burst[i]);
        t->setStartT(start[i]);
        t->Fork((VoidFunctionPtr) TestThread, (void *)NULL);
    }
}
